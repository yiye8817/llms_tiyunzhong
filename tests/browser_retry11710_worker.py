...
async function commit(retry=false){
 ...
 input.value='';const rid=++stats.requests;await fixtureNetwork({phase:'start',id:rid,prompt:stats.sentText,status:retry||mode==='plain-card'||mode==='payload-error'?200:502});
 if(!retry){failUI();if(mode==='icon-native')document.querySelector('#retry').removeAttribute('aria-label');if(mode==='stale-stop'){const stop=document.createElement('button');stop.id='stop-response-button';stop.textContent='Stop generating';document.body.append(stop);}if(mode==='plain-card'){const answer=document.querySelector('[data-role="assistant"]');answer.removeAttribute('data-role');answer.id='fixture-answer';answer.querySelector('[role="alert"]').removeAttribute('role');}if(mode==='payload-error'){document.querySelector('[data-role="assistant"]').innerHTML='<span>请求处理中</span><button id="retry" type="button" aria-label="Retry">Retry</button>';}await fixtureNetwork({phase:'finish',id:rid,body:mode==='payload-error'?JSON.stringify({error:{code:502,message:'upstream failure'}}):null});return;}
 document.querySelector('#stop-response-button')?.remove();const answer=document.querySelector('[data-role="assistant"],#fixture-answer');answer.dataset.role='assistant';answer.innerHTML='';answer.dataset.state='streaming';
...
document.addEventListener('click',e=>{
 ...
 if(target.id==='send-message-button'){
  ...
  if(mode==='never'||['native','icon-native'].includes(mode)&&!e.isTrusted||['manual','stale-stop','plain-card'].includes(mode)&&stats.retries<3)return;
    ...
    with sync_playwright() as pw:
        ...
        def event(_source,data):
            ...
            if data['phase']=='start':
                ...
                if data.get('body') is not None:emit({'event':'fixtureBody','params':{'requestId':rid,'body':data['body']}})
                emit({'event':'Network.loadingFinished','params':{'requestId':rid,'encodedDataLength':1024}})

...
const file=path.join(__dirname,'../electron/adapter.cjs'),local=createRequire(file),moduleObject={exports:{}};
...
const worker=spawn(process.env.PYTHON||'python',['-u',path.join(__dirname,'browser_send1177_worker.py')],{stdio:['pipe','pipe','inherit']});
...
(async()=>{
 ...
 try {
  ...
  const results=await Promise.all(Object.entries(adapters).map(async([id,adapter])=>{
   ...
   const result=await adapter.run({job_id:id,purpose:'candidate',prompt:text,timeout_seconds:20,total_timeout_seconds:20,submission_timeout_seconds:5,recovery_timeout_seconds:0,stable_seconds:.05,min_wait_seconds:0,input_chunk_delay_ms:10});
   assert.equal(result,id+' complete');const stats=await call('stats',{provider:id});assert.equal(stats.sends,1);assert.equal(stats.requests,1);assert.equal(stats.blockedCommits,0);assert.equal(stats.sentText,text);
   const site=sites.get(id),edits=site.commands.filter(x=>x.method==='Input.insertText');assert.ok(stats.edits >= edits.length, "one native multiline insertion may emit multiple input events");
   assert.equal(site.logs.find(x=>x.event==='adapter.request_payload').prompt_match,'exact');
   rows.push({name:id==='qwen'?'agent_long_prompt_native_chunks':'glm_hidden_rich_composer_native_send',ok:true,chars:text.length,edits:edits.length,sends:stats.sends,requests:stats.requests});return result;
  }));
 ...
 }catch(e){rows.push({name:'adapter_integration',ok:false,error:e.stack});process.exitCode=1;}
 ...
 fs.writeFileSync(process.argv[2]||'/tmp/send1177-adapter.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
})();
